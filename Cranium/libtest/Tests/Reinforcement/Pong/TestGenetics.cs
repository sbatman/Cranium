...
namespace Cranium.Lib.Test.Tests.Reinforcement.Pong
{
	...
	{
		private static readonly List<DNA> _DNAProfilesToRun = new List<DNA>();
		...
		private readonly Boolean _TeachingEnabled = true;
		...
		private void GenerateNeuralNetwork()
		{
            ...
            AF hlActivationFunction;
            if (((BooleanGene)DNAProfile.GetGene("AFHLTan")).CurrentValue) hlActivationFunction = new TanhAF();
            else hlActivationFunction = new LinearAF();
            AF conActivationFunction;
            if (((BooleanGene)DNAProfile.GetGene("AFCONTan")).CurrentValue) conActivationFunction = new TanhAF();
            else conActivationFunction = new LinearAF();
		...
		public static void Run()
		{
			...
			for (Int32 generation = 0; generation < 10; generation++)
			{
				...
				{
					Console.WriteLine($"{dna} Score : {dna.Score}");
				}
			}
		...
		private static void DNARunnerThread()
		{
			while (_DNAProfilesToRun.Count > 0)
			{
				...
				String output = $"{currentProfile} Score : {scores.Average()}";

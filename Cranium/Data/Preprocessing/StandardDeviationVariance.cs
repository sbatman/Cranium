...
namespace Cranium.Lib.Data.Preprocessing
{
    public struct DataPreprocessedStandardDeviationVariance
    {
        ...
        public Double[][] DenormalizeDataSet(Double[][] input)
        {
            for (int x = 0; x < input.Length; x++)
            {
                for (int i = 0; i < input[x].Length; i++)
                {
                    input[x][i] = ((input[x][i] * Scale[i]) * StandardDeviation[i]) + Average[i];
                }
            }
            return input;
        }
    ...
    /// <summary>
    /// This class is designed as a Dataset Preprocessor. It will take in the data provided,
    /// calculate the average and standard deviation and then return the data as a comparison
    /// against variance of the standeviation from the average. So a value of 2 would signify that
    /// it is twice the standard deviation higher than the average.
    /// </summary>
    public static class StandardDeviationVariance
    {
        /// <summary>
        /// Loads the provided file and pulls the data into a table, this is then pre-processed and
        /// returned along with the average and the standard deviation. The file must be in CSV
        /// format with each line representing one set of inputs (seperated by commas)
        /// </summary>
        ...
        public static DataPreprocessedStandardDeviationVariance ProduceDataset(String fileName)
        {
            ...
            return returnResult;
            // } catch ( Exception e ) { throw( new Exception ( "Data pre-processing failed :" +
            // e.Message ) ); }
        }

        /// <summary>
        /// Processes the provided dataset returning the preprocessed dataset, the average and
        /// standard deviation.
        /// </summary>
        /// <returns> The dataset. </returns>
        ...
        /// <summary> Does the actual pre-processing. </summary>
        /// <param name="inputData"> Input data. </param>
